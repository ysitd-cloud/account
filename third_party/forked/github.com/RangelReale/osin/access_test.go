...
func TestAccessAuthorizationCode(t *testing.T) {
	...
	sconfig.AllowedAccessTypes = AllowedAccessType{AuthorizationCode}
	...
	req.Form.Set("grant_type", string(AuthorizationCode))
...
func TestAccessRefreshToken(t *testing.T) {
	sconfig := NewServerConfig()
	sconfig.AllowedAccessTypes = AllowedAccessType{RefreshToken}
	server := NewServer(sconfig, NewTestingStorage())
	server.AccessTokenGen = &TestingAccessTokenGen{}
	resp := server.NewResponse()
	...
	req.Form = make(url.Values)
	req.Form.Set("grant_type", string(RefreshToken))
...
func TestAccessClientCredentials(t *testing.T) {
	sconfig := NewServerConfig()
	sconfig.AllowedAccessTypes = AllowedAccessType{ClientCredentials}
	server := NewServer(sconfig, NewTestingStorage())
	server.AccessTokenGen = &TestingAccessTokenGen{}
	resp := server.NewResponse()
	...
	req.Form = make(url.Values)
	req.Form.Set("grant_type", string(ClientCredentials))
	req.Form.Set("state", "a")
	req.PostForm = make(url.Values)
...
func TestExtraScopes(t *testing.T) {
	...
	if extraScopes("a,b", "b,a") == true {
		t.Errorf("extraScopes returned true with matching scopes")
	}

	if extraScopes("a,b", "b,a,c") == false {
		t.Errorf("extraScopes returned false with extra scopes")
	}
...
func (c *clientWithMatcher) GetSecret() string        { return "" }
